...
public class LoggingAspect {
    ...
    public void logBeforeControllersMethods(JoinPoint joinPoint) {
        ...
        String logMessage = String.format("At [%s] - Method [%s] called in class [%s] by user [%s]", timestamp,
    ...
    public void logAfterControllersMethods(JoinPoint joinPoint) {
        ...
        String username = getUsername();
        String timestamp = LocalDateTime.now().format(DATE_FORMATTER);
        String logMessage = String.format("At [%s] - Method [%s] Finished in class [%s] by user [%s]", timestamp,
                methodName, className,
                username);
        // System.out.println(logMessage);
        try (PrintWriter writer = new PrintWriter(new FileWriter(LOG_FILE, true))) {
